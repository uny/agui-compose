...
kotlin {
    ...
    android {
        namespace = "dev.ynagai.agui.a2ui.material3"
    ...
    sourceSets {
        ...
        commonMain.dependencies {
            // `api`: `withMaterial3A2ui` is `withA2ui` with Material 3 defaults filled in, so a
            // consumer holds every type the lower module's signature names, plus the registry
            // `a2ui-material3` publishes and the `material3` the pending renderer draws with.
            api(projects.aguiA2uiCompose)
            api(projects.aguiMaterial3)
